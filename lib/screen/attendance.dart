      ...
      if (mounted) {
        setState(() {
          ...
          for (var d in filteredData) {
            dynamic value = '${d.code}:${d.empName}';
            sugests.add(value);
          }
        });
      }
                              ...
                              for (int i = 0; i < filteredData.length; i++)
                                DataRow(
                                  cells: <DataCell>[
                                    ...
                                    DataCell(Text(
                                        filteredData[i].lateInMin.toString())),
                                    DataCell(Text(filteredData[i].attnStatus)),
                                  ],
                                ),
                            ],
                          ),
...
Future<void> getAttendanceInfo(
  ...
  final url = Uri.parse(
    "${urls.token_url}/api/Attendance/AttendanceByUser/${userName}/${current_month}01/${current_month}30",
  );
...
Future<void> getAttendanceInfoAnotherSEarch(String userName, Token token,
  ...
  final url = Uri.parse(
    "${urls.token_url}/api/Attendance/AttendanceByUser/${userName}/${start_date}/${end_date}",
  );
